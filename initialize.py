...
def file_load(path, docs_all):
    ...
    # ▼ 修正ポイント: 社員名簿.csv を特別扱い
    if file_name == "社員名簿.csv":

        # 文字コードを自動判別: まずutf-8で読んで、失敗したらcp932を試す
        try:
        ...
        # --- 修正ポイント ---
        # 部署ごとに全列をまとめて Document 化（RAG検索用）
        grouped_texts = []
        for dept, rows in df.groupby("部署"):
            # DataFrame の内容をCSV形式に変換（表として保持）
            employees_table = rows.to_csv(index=False)
            # 【修正】text = f"{dept}に所属する従業員情報は次の通りです:\n" + rows.to_csv(index=False)
            text = f"{dept}に所属する従業員情報は次の通りです:\n{employees_table}"
            grouped_texts.append(text)
        # 全部署分を1つのドキュメントに統合
        full_text = "\n\n".join(grouped_texts)
        # Documentオブジェクトに変換して追加
        docs_all.append(Document(page_content=full_text, metadata={"source": path}))

        # ★ DataFrame をそのまま session_state に保存（表表示用）
        st.session_state["employee_df"] = df

        return  # ★ここで終了（通常処理に進まない）   
