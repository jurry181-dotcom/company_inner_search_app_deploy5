...
import pandas as pd  # ★追加: DataFrame変換用　★残す: 念のため DataFrame処理で利用する
#【修正】import io            # ★追加: CSV文字列をDataFrameに変換するため　★削除可: CSV文字列パースは不要になる（社員名簿を直接使うため）
...
def display_select_mode():
    ...
    # 【変更後】
    # サンプル2に合わせて、サイドバーにラジオボタンを移動
    # --- 変更理由 ---
    #   画面の中央ではなく、サイドバーにモード選択を配置して使いやすくするため。
    with st.sidebar:
        st.header("利用目的")
        st.session_state.mode = st.radio(
            label="",
            options=[ct.ANSWER_MODE_1, ct.ANSWER_MODE_2],
            index=0  # デフォルトで最初の選択肢を選ぶ
        )

        # 【変更箇所】
        # --- 変更理由 ---
        #   ラジオボタンの下に区切り線を追加し、
        #   その下に説明文を配置して利用者に分かりやすくするため。
        st.divider()  # 区切り線を追加
...
def display_initial_ai_message():
    ...
    with st.chat_message("assistant"):
        ...
        st.warning("⚠️ 具体的に入力したほうが適切な回答を得やすいです。")
            ...
            # LLMからの回答の場合
                ...
                if message["content"]["mode"] == ct.ANSWER_MODE_1:
                    ...
                    if not "no_file_path_flg" in message["content"]:
                        ...
                        # ★修正: ページ番号が保存されている場合は追記して表示
                        # 参照元ドキュメントのページ番号が取得できた場合にのみ、ページ番号を表示
                        if "main_page_number" in message["content"]:
                            st.success(f"{message['content']['main_file_path']} （ページNo.{message['content']['main_page_number']}）", icon=icon) # ★修正表示
                        else:
                        ...
                        if "sub_message" in message["content"]:
                            ...
                            for sub_choice in message["content"]["sub_choices"]:
                                ...
                                if "page_number" in sub_choice:
                                    st.info(f"{sub_choice['source']} （ページNo.{sub_choice['page_number']}）", icon=icon)  # ★修正表示
                                else:
...
def display_search_llm_response(llm_response):
    ...
    if llm_response["context"] and llm_response["answer"] != ct.NO_DOC_MATCH_ANSWER:
        ...
        # ★修正: （ページNo.{main_page_number}）ページ番号が取得できた場合は「（ページNo.X）」を追記して表示
        # ページ番号が取得できた場合のみ、ページ番号を表示（ドキュメントによっては取得できない場合がある）
        if "page" in llm_response["context"][0].metadata:
            # ページ番号を取得
            # ★修正: LLMが返すpageは0始まりのindexなので、表示用に1始まりへ補正
            main_page_number = llm_response["context"][0].metadata["page"]+1
            # 「メインドキュメントのファイルパス」と「ページ番号」を表示
            st.success(f"{main_file_path} （ページNo.{main_page_number}）", icon=icon)  # ★修正表示
        else:
        ...
        for document in llm_response["context"][1:]:
            ...
            # ★修正: サブ文書もページ番号があれば保持する（後で表示＆会話ログ保存に使う）
            # ページ番号が取得できない場合のための分岐処理
            if "page" in document.metadata:
                # ページ番号を取得
                # ★修正: サブ候補も同様に0→1へ補正して保存（後で表示に使う）
                sub_page_number = document.metadata["page"]+1
                # 「サブドキュメントのファイルパス」と「ページ番号」の辞書を作成
                sub_choice = {"source": sub_file_path, "page_number": sub_page_number}  # ★修正保持
            else:
        ...
        # ★修正: メインのページ番号も会話ログに保存（再描画時に活用）
        if "page" in llm_response["context"][0].metadata:
            content["main_page_number"] = main_page_number # ★修正保持
...
def display_contact_llm_response(llm_response):
    ...
    # 【変更後】
        # ★修正箇所 ここから ----------------------------
    # LLM回答に含まれる部署名を検出 → DataFrameをフィルタして表示
    dept = None
    if "employee_df" in st.session_state:  # initialize.py で保存した社員名簿
        for d in st.session_state["employee_df"]["部署"].unique():
            if d in content:  # LLM回答に部署名が含まれていれば採用
                dept = d
                break

    if dept:
        df = st.session_state["employee_df"]
        filtered = df[df["部署"] == dept]  # 部署ごとに抽出
        st.subheader(f"{dept}に所属している従業員情報一覧")
        st.dataframe(filtered, use_container_width=True)  # 横スクロール可能な表
    else:
        # 部署が特定できなかった場合は回答文をそのまま表示
        st.markdown(content)
    # ★修正箇所 ここまで ----------------------------
    ...
    if llm_response["answer"] != ct.INQUIRY_NO_MATCH_ANSWER:
        ...
        for document in llm_response["context"]:
            ...
            if "page" in document.metadata:
                # ページ番号を取得
                # ★修正: 情報源リストでも0→1へ補正して表示用文字列を作る
                page_number = document.metadata["page"]+1
                # 「ファイルパス」と「ページ番号」
                file_info = f"{file_path} （ページNo.{page_number}）"  # ★修正表示用文字列
            else:
                # 「ファイルパス」のみ
