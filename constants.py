...
# ★追加: テキストファイル読込時のエンコーディングを定数化
#   - 文字化けを避けるためにUTF-8を明示。
TEXT_FILE_ENCODING = "utf-8"  # ★追加

SUPPORTED_EXTENSIONS = {
    ".pdf": PyMuPDFLoader,
    ".docx": Docx2txtLoader,
    ".csv": lambda path: CSVLoader(path, encoding="utf-8"),
    # ★追加: .txt を TextLoader で読み込めるようにする
    #   - 既存の取り込み処理は SUPPORTED_EXTENSIONS[拡張子](path) の想定なので
    #     他コードの修正は不要（encoding だけ指定）。
    ".txt": lambda path: TextLoader(path, encoding=TEXT_FILE_ENCODING),  # ★追加
}
